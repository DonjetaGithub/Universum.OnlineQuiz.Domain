...
function Delete(deleteUrl) {
    swal({
        title: "Jeni i sigurt që do ta fshini kuizin?",
        text: "Pasi të fshihet, nuk do të mund të riktheni",
        icon: "paralajmërim",
        buttons: true,
        dangerMode: true
    }).then((willDelete) => {
        // nese perdoruesi thote PO
        if (willDelete) {
            $.ajax({
                type: "Fshije",
                url: deleteUrl,
...
function CompleteQuiz() {
    ...
    if (elemList.length != 4) {
        toastr.error("Ju lutemi plotësoni kuizin");
